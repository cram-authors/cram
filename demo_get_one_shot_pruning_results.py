...
from utils import get_datasets, imagenet_calibration_datasets, load_checkpoint, percentile
from utils import one_shot_prune, test_sparsity
...
def get_parser():
    ...
    parser.add_argument('--gpu', default=0, type=int, help='ID of the GPU used')
    parser.add_argument('--unif', action='store_true', help='use uniform pruning; uses global if false')
    parser.add_argument('--use_calib', action='store_true', help='use calibration dataset for BN tuning; uses the entire train set if false')
    parser.add_argument('--fixed_calib', action='store_true', help='use FIXED calibration dataset for BN tuning')
    parser.add_argument('--calib_size', type=int, default=1000, help='# samples / class for the calibration set')
...
if __name__=='__main__':
    ...
    # args.use_calib should be True 	
    if args.use_calib:
        if args.fixed_calib:
	    # use fixed calibration set for BNT (only for ImageNet) --> first call generate_calibration_dset.py to create the calibration set
	    # args.calib_size should be 1 here
            data_train, data_test = imagenet_calibration_datasets('PATH/TO/CALIB', 'IMAGENET_PATH', calib_size=args.calib_size)
        else:
    ...
    if args.sparsity>0:
        print(f'Pruning the model to {100*args.sparsity}% sparsity')
