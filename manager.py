...
from utils import preprocess_for_device, load_checkpoint, save_checkpoint, TrainingProgressTracker
...
class Manager:
    ...
    def __init__(self, args):
        ...
        # Define datasets
        self.data_train, self.data_test, self.data_val = get_datasets(*self.data, use_val_split=self.use_train_val_split)
        self.train_loader = DataLoader(self.data_train, batch_size=self.batch_size, shuffle=True,
                                       num_workers=self.num_workers)
        if self.data_val is not None:
            # Use the validation set for testing periodically during training
            print('Using the validation set for testing!')
        ...
        self.optimizer, self.lr_scheduler, self.use_sam = build_training_schedule_from_config(self.model, self.config, sam_rho=args.sam_rho)
        ...
        if args.l2_reg_loss:
            l2_reg = args.l2_reg
            def l2_reg_loss(output, target):
                loss = F.cross_entropy(output, target)
                model_params = list(self.model.named_parameters())
                for i, (n, w) in enumerate(model_params):
                    loss += 0.5 * l2_reg * torch.sum(w ** 2)
                return loss
            self.loss_fn = l2_reg_loss
    ...
    def log_eval_stats(self, epoch, eval_loss, eval_acc, eval_correct, type='val'):
        self.logging_function({'epoch': epoch, type + ' loss':eval_loss, type + ' acc':eval_acc})
        logging.info({'epoch': epoch, type + ' loss':eval_loss, type + ' acc':eval_acc})
        if type == "val":
    ...
    def optimize_minibatch(self, minibatch):
        ...
        if self.use_sam:
            ...
            if hasattr(self.optimizer, "sparsify_weights_first"):
                self.optimizer.sparsify_weights_first()
            ...
            self.loss_fn(self.model(in_tensor), target).backward()
    ...
    def run(self):
        ...
        for epoch in range(self.initial_epoch, self.n_epochs):
            ...
            self.log_eval_stats(epoch, val_loss, val_acc, val_correct, type='val')
