...
def one_shot_prune(model, sparsity):
    ...
    for name, m in model.named_parameters():
        if len(m.shape)>1:
            masks_dict[name] = (m.data.abs() > threshold).float()
            m.data *= masks_dict[name]
    return masks_dict


def one_shot_uniform_prune(model, sparsity, skip_first_last=True):
    masks_dict = {}
    all_params = list(model.named_parameters())
    for i in range(1, len(all_params)-2):
        param = all_params[i][1]
        if len(param.shape)<2: continue
        param_stats = param.data.abs().view(-1)
...
def one_shot_NM_prune(model, N, M, skip_last=False):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            group = int(length / M)
            weight_temp = module.weight.detach().abs().permute(0,2,3,1).reshape(group, M)
        ...
        if (not skip_last) and isinstance(module, nn.Linear):
            length = module.weight.data.numel()
            group = int(length / M)
            weight_temp = module.weight.detach().abs().reshape(group, M)
            index = torch.argsort(weight_temp, dim=1)[:, :int(M-N)]
